							...
							<div class="row">
								...
								<div class="col-md-5">
									...
									<div class="input-group input-group-sm">
										...
										<?php if ($canned_message['success']): ?>
										
											<?php foreach($canned_message['data'] as $value): ?>
											
												<input type="text" name="canned_message" class="form-control" placeholder="Canned message" value="<?php echo $value['canned_message']; ?>">
						...
						<?php if ($canned_messages['success']): ?>
							...
							<div class="table-responsive">
								...
								<form method="post" action="<?php echo $absolute_url; ?>admin/canned_messages/delete">
									...
									<table class="table">
										<thead>
											<tr>
												<th><input type="checkbox" id="select-all" value="cb_message"></th>
										...
										<tbody>
											<?php foreach($canned_messages['data'] as $value): ?>
												<tr>
													<td><input type="checkbox" name="cb_message[]" value="<?php echo $value['message_id']; ?>"></td>
							...
							<ul class="pager">
								<?php if ($canned_messages['current_page'] > 1 && ($canned_messages['current_page'] - 1) < $canned_messages['pages']): ?>
									<li><a href="<?php echo $absolute_url; ?>canned_messages/page/<?php echo ($canned_messages['current_page'] - 1); ?>"><?php echo get_text('previous'); ?></a></li>
								<?php endif; ?>
								<?php if ($canned_messages['pages'] > $canned_messages['current_page'] && ($canned_messages['current_page'] - 1) < $canned_messages['pages']): ?>
									<li><a href="<?php echo $absolute_url; ?>canned_messages/page/<?php echo ($canned_messages['current_page'] + 1); ?>"><?php echo get_text('next'); ?></a></li>
								<?php endif; ?>
							</ul>
